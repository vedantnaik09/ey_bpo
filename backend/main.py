...
async def get_complaints_by_category(
    category: str,
    # current_user: CurrentUser = Depends(get_current_user) # optional
):
    """Get complaints filtered by category. Currently not restricted by role."""
    try:
        df = db.get_complaints()
        ...
        return [
            {
                **row.to_dict(),
                ...
                if pd.notna(row["scheduled_callback"])
                else None,
            }
            for _, row in df.iterrows()
        ]
